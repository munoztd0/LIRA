...
COVA = "+ age + gender + BMI_t1  + thirsty  + hungry +"
random = '(phase*session|id) + (1|trial)'

## FACTORIAL EARLY/LATE LEARNING PHASIS
facmod <- lmer(as.formula(paste(measurevar, paste('phase',formula, COVA, random), sep="~")),data=INST, control = control, REML = FALSE)

## PIECEWISE REGRESSION WITH SPLINES
splinemod <- lmer(as.formula(paste(measurevar, paste('lspline(Trial, 5)', formula, COVA, random), sep="~")),data=INST, control = control, REML = FALSE)

BF_fit = as_tibble(bayesfactor_models(facmod, splinemod, denominator = facmod)); 
```
\
...
model = mixed(as.formula(paste(measurevar, paste('Trial1*intervention*session + Trial2', formula, COVA, random), sep="~")), data = INST, method = "LRT", control = control, REML = FALSE); model 

table = nice(model);  ref_grid(model)  #triple check everything is centered at 0

#calculate exclusion BF01 for each effect

test = extractBF(generalTestBF(as.formula(paste(measurevar, paste('Trial1*intervention*session + Trial2', formula, COVA,  "id"), sep="~")), data= INST.means, whichRandom = c('id'), neverExclude =  'id', whichModels ="top")); BF = 1/test[1] #switch to BF10 inclusion)); BF = 1/test[1] #switch to BF10 inclusion
table$BF10 = rev(BF$bf) # reorder to match lmer model
...
pp <- ggplot(INST.means, aes(x = cond, y = grips, 
  ...
  scale_fill_manual(values=c("1"= pal[4], "-1"=  pal[1]), guide = 'none') +
  scale_color_manual(values=c("1"= pal[4], "-1"=  pal[1]), guide = 'none') +
  scale_shape_manual(name="Intervention", labels=c("Placebo", "Liraglutide"), values = c(1, 2)) +
  facet_wrap(~session, labeller=labeller(session = labels)) +
  theme_bw()


pp + html_theme +theme(legend.title  = element_text(size = 9),legend.text  = element_text(size = 9),legend.position=c(.95,.98))
plt = pp + averaged_theme +theme(legend.title  = element_text(size = 10),legend.text  = element_text(size = 8),legend.position=c(.95,.98))
```
...
dfTRIAL <- summarySEwithin(INST, na.rm=TRUE,
                           measurevar = "grips",
...
dfTRIALg <- summarySEwithin(INST, na.rm=TRUE,
                            measurevar = "grips",
                            withinvars = c("trial","session"),
