...
aov_car(data = dftout1, inter ~ intervention*GLP1 + intervention*Fast_glu + intervention*bmi + intervention*gender + intervention*age + intervention*piss + intervention*thirsty + intervention*hungry + intervention*insulin + intervention*AEA + intervention*PEA + intervention*OEA + Error(id), factorize = F)

mod1 = lm(data = dftout1, inter ~ intervention*GLP1 + intervention*Fast_glu + intervention*bmi + intervention*gender + intervention*age + intervention*piss + intervention*thirsty + intervention*hungry + intervention*insulin + intervention*AEA + intervention*PEA + intervention*OEA )
...
plot1 = interact_plot(mod1,pred = "Fast_glu", modx = "intervention",  interval = T, plot.points = T,facet.modx = T) 
plt = plot1 + 
  geom_point(data =x, aes(x = Fast_glu, y = inter, group=NULL))+
  ylab('') +
  xlab('Fasting glucose') +
  scale_y_continuous(expand = c(0, 0), breaks = c(seq.int(-2,3, by = 1)), limits = c(-2.3,3.3)) +
  scale_x_continuous(expand = c(0, 0), breaks = c(seq.int(-1.5,1.5, by = 1)), limits = c(-2,2)) +
  scale_linetype_manual(name="Intervention", values=c(1,1), na.value="blank", guide = 'none')+
  scale_fill_manual(values=c("0"= pal[1], "1"=  pal[6]), guide = 'none') +
  scale_color_manual(labels=c("Placebo", "Liraglutide"), values=c("intervention = 0"=pal[1], "intervention = 1"=pal[6]))  +
  theme_bw()+ facet_wrap(~intervention, labeller=labeller(intervention = labels))

ppp0 <- plt + averaged_theme
ppp0
...
pp <- ggplot(dftout1, aes(x = Fast_glu, y = inter, fill = intervention, color = intervention)) +
  geom_hline(yintercept = 0, linetype = 2) + 
  geom_point(data = dfH,aes(x =0), alpha = 0.5) +
...
figure = annotate_figure(ppp1, bottom = text_grob("Error bars show SE // intervention: p-value = 0.047  // intervention:Fasting glucose: p-value = 0.0052",  vjust = -0.5, hjust = -0.3, x = 0.4, face = "italic", size = 10))

figure

cairo_pdf(file.path(figures_path,'Figure_HEDONIC_hpp_con_fMRI.pdf'))
print(figure)
dev.off()
