...
This file was automatically created via `the Repro package (version 0.1.0)` using  `r x$platform[1]`
...
table = nice(model);  ref_grid(model)  #triple check everything is centered at 0
table = table[c(1, 4, 2, 3, 5, 6, 7, 8, 9, 10, 11, 13, 12, 14, 15, 16),]
#calculate exclusion BF01 for each effect (had to change to trial instead of ID)
# test = extractBF(generalTestBF(as.formula(paste(measurevar, paste('Trial1*intervention*session + Trial2', formula, COVA,  "trial"), sep="~")), data= INST.means, whichRandom = c('trial'), neverExclude =  'trial', whichModels ="top")); BF = 1/test[1] #switch to BF10 inclusion)); BF = 1/test[1] #switch to BF10 inclusion
# table$BF10 = rev(BF$bf) # reorder to match lmer model
...
tables2[1:length(table$Effect),1:4] %>% kbl(caption ="Grips", ) %>%
  kable_styling(latex_options = "HOLD_position", position = "center", full_width = F) %>%  row_spec(0,bold=T,align='c')
tmp = tables2[(length(table$Effect)+1):(length(table$Effect)+4),1:2]
names(tmp) <- NULL
tmp %>% kbl(digits = 2) %>%
  kable_styling(latex_options = "HOLD_position", position = "center", full_width = F) 
```
...
pp <- ggplot(dfTRIAL, aes(x =trial, y = grips)) +
  ...
  scale_x_continuous(expand = c(0, 0),  limits = c(0,15.1),  breaks=c(1,5,10,15)) +
...
#pp + html_theme + theme(legend.position='none', axis.text.x = element_text(size = 16))
pltpre = pp + averaged_theme + theme(axis.text.x= element_text(size = 14), axis.text.y= element_text(size = 14), legend.position='none', plot.title = element_text(hjust = 0.5, size = 20))
#-------------------------------POST
...
#pp + html_theme + theme(legend.position='none', axis.text.x = element_text(size = 16))
pltpost = pp + averaged_theme + theme(axis.text.x= element_text(size = 14), axis.text.y= element_text(size = 14, color="white"), legend.position=c(.94,.93), plot.title = element_text(hjust = 0.5, size = 20))
...
bottom = text_grob("Trial", size = 16),
 left = text_grob("Number of Grips", rot = 90, size = 16),
)
plt
...
Mobilized effort = Area under the curve of the force exerted exceeding the delivery threshold during Pavlvovian cue presentation
condition = CS+ or CS-
...
#calculate exclusion BF01 for each effect
# test = extractBF(generalTestBF(as.formula(paste(measurevar, paste (formula, COVA, 'id'), sep="~")), data= PIT.means, whichRandom = 'id', neverExclude =  'id', whichModels ="top")); BF = 1/test[1] #switch to BF10 inclusion)); BF = 1/test[1] #switch to BF10 inclusion
# table$BF10 = rev(BF$bf) # reorder to match lmer model

mod =  lmer(as.formula(paste(measurevar, paste (formula, COVA, random), sep="~")), data = PIT, control = control, REML = FALSE)
...
tables2 <- as.matrix(tables2) %>% as_tibble()
tables2[is.na(tables2)] <- ""
tables2[1:length(table$Effect),1:4] %>% kbl(caption ="Mobilized effort (a.u.)", ) %>%
  kable_styling(latex_options = "HOLD_position", position = "center", full_width = F) %>%  row_spec(0,bold=T,align='c')
tmp = tables2[(length(table$Effect)+1):(length(table$Effect)+5),1:2]
names(tmp) <- NULL
tmp %>% kbl(digits = 2) %>%
...
pp + html_theme +theme(legend.title  = element_text(size = 9),legend.text  = element_text(size = 9),legend.position=c(.94,.94))
plt = pp + averaged_theme +theme(legend.title  = element_text(size = 10),legend.text  = element_text(size = 8),legend.position=c(.94,.94))
```
...
Perceived liking = how pleasant is the liquid solution rated (0-100, with repetitions)   &   condition = Milshake or Tasteless   &   intensity = how intense the liquid solution were rated 
```{r HED, echo=FALSE, include=FALSE, cache=TRUE}
measurevar = "lik"
formula = "condition*intervention*session"
COVA = "+ age + gender + BMI_t1  + thirsty  + hungry + int  + "
random = " (condition*session|id) + (1|trialxcondition)"
...
#calculate exclusion BF01 for each effect
# test = extractBF(generalTestBF(as.formula(paste(measurevar, paste (formula, COVA, 'id'), sep="~")), data= HED.means, whichRandom = 'id', neverExclude =  'id', whichModels ="top")); BF = 1/test[1] #switch to BF10 inclusion)); BF = 1/test[1] #switch to BF10 inclusion
# table$BF10 = rev(BF$bf) # reorder to match lmer model

mod =  lmer(as.formula(paste(measurevar, paste (formula, COVA, random), sep="~")), data = HED, control = control, REML = FALSE)
...
tables2 <- as.matrix(tables2) %>% as_tibble()
tables2[is.na(tables2)] <- ""
tables2[1:length(table$Effect),1:4] %>% kbl(caption ="Perceived liking", ) %>%
  kable_styling(latex_options = "HOLD_position", position = "center", full_width = F) %>%  row_spec(0,bold=T,align='c')
tmp = tables2[(length(table$Effect)+1):(length(table$Effect)+5),1:2]
names(tmp) <- NULL
tmp %>% kbl(digits = 2) %>%
...
## Perceived liking (Taste) by condition
...
pp + html_theme +theme(legend.title  = element_text(size = 9),legend.text  = element_text(size = 9),legend.position=c(.70,.15))
plt = pp + averaged_theme +theme(legend.title  = element_text(size = 5),legend.text  = element_text(size = 8),legend.position=c(.97,.15))
```
