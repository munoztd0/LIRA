...
<<<<<<< HEAD
  packages: [ aaronpeikert/repro@devel, crsh/papaja@devel, tinylabels, apaTables, MBESS, afex, ggplot2, ggpubr, Rmisc, emmeans, tidyr, BayesFactor, bayestestR, devtools, lspline, kableExtra, sjPlot, knitr, XML, rlist, janitor, optimx, ggthemes, dplyr, JWileymisc, corrplot, glmnet, intmed, stringr, cowplot ]
=======
  packages: [ aaronpeikert/repro@devel, crsh/papaja@devel, tinylabels, apaTables, MBESS, afex, ggplot2, ggpubr, Rmisc, emmeans, tidyr, BayesFactor, bayestestR, devtools, lspline, kableExtra, sjPlot, knitr, XML, rlist, janitor, optimx, ggthemes, dplyr, JWileymisc, corrplot, glmnet, intmed, stringr, cowplo, pander]
>>>>>>> 42279c50827469e90e4fe33c2e4f7c599ff95c43
  scripts: R/clean.R
  apt:
    - libgsl0-dev
---
...
This file was automatically created via `the Repro package (version 0.1.0)` using  `r session$platform[1]`
...
niter = 5000; warm = 1000; chains = 4; cores = 4 # number of iterations (to change if you want to quick check and warmups #or also parallel::detectCores()/2)
options(scipen = 666, warn=-1, contrasts=c("contr.sum","contr.poly"), mc.cores = cores); cl <- cores  #remove scientific notation # remove warnings #set contrasts to sum !
...
sizes = 1:length(dfmed[c(-1)])
len = length(sizes)
seeds <- vector(mode = "list", length = len)
for(i in 1:(len-1)) seeds[[i]]<- sample.int(n=1000, size = length(sizes)+1)
# for the last model
seeds[[len]]<-sample.int(1000, 1)
...
# x<-model.matrix(intervention~.,data=dfmed); x=x[,-1]
# 
# fit<-cv.glmnet(x=x,y=dfmed$intervention,family ="binomial",type.measure='deviance',nfolds=5,alpha=.5)
# plot(fit)
# 
...
med_res <- mediate(y = "weightLoss", med = c("X2.AG_diff" ,  "reelin_diff"  ,"MCP_diff" ), c = c("age","gender"), treat = "Intervention", ymodel = "regression", mmodel = c("regression", "regression", "regression"), treat_lv = 1, control_lv = 0, incint = TRUE, inc_mmint = FALSE, conf.level = 0.95, data = df, sim = 1000, complete_analysis = TRUE, digits = 3,  summary_report=F)

tables1 <- list.clean(readHTMLTable("res.html"), fun = is.null, recursive = FALSE)

med_res <- mediate(y = "weightLoss", med = c( "TNFalpha_diff",  "GLP_diff" ,   "Glu_diff"  ), c = c("age","gender"), treat = "Intervention", ymodel = "regression", mmodel = c("regression", "regression", "regression" ), treat_lv = 1, control_lv = 0, incint = TRUE, inc_mmint = FALSE, conf.level = 0.95, data = df, sim = 1000, complete_analysis = TRUE, digits = 3, summary_report=F)

tables2 <- list.clean(readHTMLTable("res.html"), fun = is.null, recursive = FALSE)
```
