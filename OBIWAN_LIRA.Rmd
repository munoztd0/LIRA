...
con_ob = contrast(ems$emmeans, method = list("CS+ > CS-: Obese" = HED_obe)) #HED ob
con_lean =contrast(ems$emmeans, method = list("CS+ > CS-: Lean" = HED_lean), ) #HED lean
...
\
\
```{r HED_save, echo=FALSE, include = FALSE, warning=FALSE}
...
```
```{bash, include=F}
cp OBIWAN_LIRA.html index.html #this is just to change the output name for github
```
