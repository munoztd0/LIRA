...
table$BF10 = BF$bf[order(c(15,14,13,12,11,10,9,7,6,5,4,3,2,1,8))] # reorder
  ...
  tab_model(mod, show.p = F,show.intercept = F, show.se = T, title ="", show.re.var = F, digits = 2, dv.labels = "Latency", emph.p = TRUE, file = "tmp/temp1.html")#pred.labels=c("CS+", "Lean", "Interaction (Lean:CS+)")
```
\
```{r PAV_RT_mod}
...
tab_model(mod, show.p = F,show.intercept = F, show.se = T,  title ="", show.re.var = F, digits = 2, dv.labels = "Pleasantness Ratings (Pavlovian Cue)", emph.p = TRUE, file = "tmp/temp2.html")#pred.labels=c("CS+", "Lean", "Interaction (Lean:CS+)")
```
```{r PAV_lik_res}
...
tables2 <- as.matrix(tables2) %>% as_tibble()
tables2[is.na(tables2)] <- ""
tables2[1:15,1:4] %>% kbl(caption ="Pleasantness Ratings (Pavlovian Cue)" ) %>%
  kable_styling(latex_options = "HOLD_position", position = "center", full_width = F) %>%  row_spec(0,bold=T,align='c')
tmp =tables2[16:17,1:2]
names(tmp) <- NULL
tmp %>% kbl() %>%
...
tab_model(mod, show.p = F,show.intercept = F, show.se = T, title ="", show.re.var = F, digits = 2, dv.labels = "Number of grips", emph.p = TRUE, file = "tmp/temp3.html") # pred.labels=c("trial &#60 5", "trial > 5", "group[1]", "age", "gender[1]", "thirsty", "hungry", "trial &#60 5 * group[1]", "trial > 5 * group[1]")
```
```{r INST_mod_tab}
...
model = aov_car(AUC ~ condition*intervention + age + gender + thirsty + hungry + BMI_t1  + diff_base + Error(id/condition), data= PIT.means, factorize = F, anova_table = list(correction = "GG", es = "none"))
table = nice(model); 
...
interactions::interact_plot(mod,pred = 'diff_base', modx = 'condition')
...
# 
# df_est = emmeans(mod, pairwise~ condition|session) # estimate contrasts means by group from the model 
...
model = aov_car(liking ~ condition*intervention + age + gender + thirsty + hungry + BMI_t1  + diff_base + int + Error(id/condition), data= HED.means, factorize = F, anova_table = list(correction = "GG", es = "none"))
table = nice(model); 
