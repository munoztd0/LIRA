...
numer <- c("OEA", "PEA","X2.AG","AEA","Leptin",  "Resistin","adiponectin","MCP","TNFalpha","reelin","glucagon", "Ghrelin","obestatin","GLP1","insulin","Fast_glu")

PAV.means <- aggregate(PAV[,c(numer)] , by = list(PAV$id, PAV$condition,PAV$session), FUN = 'mean',na.action = na.omit)
...
PAV.means = PAV.means %>% gather(variable, value, all_of(numer)) %>%  unite(var, variable,Group.3) %>% spread(var, value)
x = colnames(PAV.means)
x[1:2]<- c('id','condition')
colnames(PAV.means) = x
...
# creates diff BMI for each data
PAV.means = diffXglu(PAV.means)
PAV.means = diffX(PAV.means)
df = subset(PAV.means, condition ==  "CSminus")
dft = merge(df, info, by = "id")
# dflist <- lapply(mget(tables),function(x) diffXglu(x))
# list2env(dflist, envir=.GlobalEnv)
